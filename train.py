...
# 抑制特定的UserWarning关于padding mask的警告
warnings.filterwarnings("ignore", message="Padding mask is disabled when using scaled_dot_product_attention.*")
# 抑制albumentations版本检查超时警告
...
from videox_fun.data.dataset_image_video import (ImageVideoDataset,
                                                 ImageVideoSampler,
                                                 get_random_mask)
from videox_fun.models import (AutoencoderKLWan3_8, CLIPModel, WanT5EncoderModel,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save folder to the mask files."
    )
...
def overwrite_with_unrelated_mask(mask_file_folder, pixel_values,
                                  mask_pixel_values, mask, nohuman,
                                  use_mask_union=True):
    raise NotImplemented("Function `overwrite_with_unrelated_mask` not implemented yet.")
...
def main():
    ...
    def signal_handler(signum, frame):
        ...
        logger.warning(f"Received User Signal {signum}, Process is terminated...")
        # 这里可以添加清理代码
        sys.exit(0)
    ...
    print(f"deepspeed_plugin status: {deepspeed_plugin}")
    ...
    if accelerator.is_main_process:
        print(f"Tensorboard logging_dir location: {logging_dir}")
        writer = SummaryWriter(log_dir=logging_dir, flush_secs=60)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                if args.train_mode != "normal": # here we use inpaint mode
            ...
            with accelerator.accumulate(transformer3d):
                ...
                if args.mask_file_folder is not None:
                    mask_pixel_values, mask = overwrite_with_unrelated_mask(
                        mask_file_folder=args.mask_file_folder,
                        pixel_values=pixel_values,
                        mask_pixel_values=mask_pixel_values,
                        mask=mask,
                        nohuman=True,  # specially set this value
                        use_mask_union=args.use_mask_union,
                    )
                ...
                if config['vae_kwargs'].get('spatial_compression_ratio', 16) >= 16:
